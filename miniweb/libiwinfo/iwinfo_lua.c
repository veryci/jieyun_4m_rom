...
static int iwinfo_L_get_phy_connect(lua_State *L)
{
	const char *ifname = luaL_checkstring(L, 1);
	//const char *type = iwinfo_type(ifname);
	const int up = net_detect(ifname);
	lua_newtable(L);
	lua_pushnumber(L, 1);
	lua_pushnumber(L, up);
	lua_settable(L, -3);
...
static const luaL_reg R_common[] = {
	{ "type", iwinfo_L_type },
	{ "get_phy_connect", iwinfo_L_get_phy_connect },
	{ "get_wisp_connect", iwinfo_L_get_phy_connect },
	{ "__gc", iwinfo_L__gc  },
	{ NULL, NULL }
};
