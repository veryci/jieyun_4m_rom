...
function upgrade()
    ...
    luci.sys.call("check_upgrade > /dev/null")
    ...
    if retstate == "1" and errcode == "0" then
        ...
        if url == "" then
            ...
            if ret == 0 then
                ...
                if ret == 0 then
        	    local newver = uci_t:get("onekeyupgrade","config","newversion") or "1.0.0.0"
        	    luci.sys.call("uci set system.system.fw_ver=%s" % newver)
...
function save()
    ...
    luci.sys.fork_exec("/sbin/upgrade_rule");
    luci.sys.fork_exec("/etc/init.d/cron restart &>/dev/null");
...
function recheck()
    ...
    info.detaile = fs.readfile("/tmp/verdesc"):gsub("\\","\\\\") or "#x65b0#x7248#x672c#x53d1#x5e03"
    luci.http.prepare_content("application/json")
    luci.http.write_json(info)
end
