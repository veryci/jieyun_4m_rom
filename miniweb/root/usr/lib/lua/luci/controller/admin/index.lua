...
function index()
	...
	entry({"admin", "freezero"}, call("action_freezero"), _("Freezero"), 9)
...
function action_freezero()
	local uci_t = uci.cursor()
	local ifname = uci_t:get("network","wan","ifname")
	luci.http.redirect("/fzcgi/freezero.cgi?net="..ifname.."&id=jy")
end
...
function gra_checkversion()
	...
	if pop_upgrade == "1" then
		...
		local connect = luci.util.exec("ping -c 1 -W 1 www.baidu.com | grep -w \"0% packet loss\"")
	...
	local nowversion = luci.veryci.softversion
