...
function interface._ubus_pre(self, field)
	if not _ubusdevcache[self.ifname] then
		_ubusdevcache[self.ifname] = _ubus:call("network.device", "get_pre",
		                                        { name = self.ifname })
	end
	if _ubusdevcache[self.ifname] and field then
		return _ubusdevcache[self.ifname][field]
...
--  add tx/rx rate interface by chengjun.tao
function interface.tx_rate(self)
	local stat = self:_ubus("statistics")
	return stat and stat.tx_rate or 0
end

function interface.rx_rate(self)
	local stat = self:_ubus("statistics")
	return stat and stat.rx_rate or 0
end

function interface.tx_rate_pre(self)
	local stat = self:_ubus_pre("statistics")
	return stat and stat.tx_rate or 0
end

function interface.rx_rate_pre(self)
	local stat = self:_ubus_pre("statistics")
	return stat and stat.rx_rate or 0
end
--
