...
--- Get the given option from the n section with the given type.
-- @param config	UCI config
-- @param type		UCI section type
-- @param option	UCI option (optional)
-- @param default	Default value (optional)
-- @return			UCI value
function Cursor.get_num_by_name(self, conf, stype, base, name_check)
	local rv = nil
	local num = 0
	self:foreach(conf, stype,
		function(s)
			if s[base] == name_check then
				rv = num
				return false
			end
			num = num + 1
...
function Cursor.get_name_by_ifname(self, conf, stype, base, name_check)
	local rv = nil
	self:foreach(conf, stype,
		function(s)
			if s[base] == name_check then
