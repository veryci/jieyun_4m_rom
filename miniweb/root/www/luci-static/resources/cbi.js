...
var cbi_validators = {
    ...
    'macaddr': function()
    {
        if(this.match(/^([a-fA-F0-9]{2}:){5}[a-fA-F0-9]{2}$/))
        {
            if(this == "00:00:00:00:00:00" || this.toUpperCase() == "FF:FF:FF:FF:FF:FF")
            {
                return error_msg[29][1];
            }
            else if((parseInt(this,16) & parseInt('01',16)) != 0)
            {
                return error_msg[30][1];
            }
            else
                return true;
        }
    ...
    'wpakey_minlen_5': function()
    {
        var v = this;
        if((v.length >= 5) && (v.length <= 64)){
            for(var i = 0; i < v.length; i++){
                    if(v.charAt(i) < ' ' || v.charAt(i) > '~'){
                            return error_msg[28][1];
                    }
            }
            return true;
    ...
    'wepkey_ascii': function()
    {
        ...
        if( (v.length == 5) || (v.length == 13) ){
            for(var i = 0; i < v.length; i++){
                if(v.charAt(i) < ' ' || v.charAt(i) > '~'){
                   return error_msg[25][1];
                }
            }
            return true;
        }
        else{
            return error_msg[24][1];
        }
    },
    ...
    'wepkey_hex': function()
    {
        ...
        if( v.length == 10 ){
            if(v.match(/^[a-fA-F0-9]{10}$/) != null)
                return true;
            else
                return error_msg[27][1];
        }
        else if( v.length == 26 ){
            if(v.match(/^[a-fA-F0-9]{26}$/) != null)
                return true;
            else
                return error_msg[27][1];
        }
        else{
            return error_msg[26][1];
        }
    },
    ...
    'ipv4lan':function()
    {
     ...
     'checkPPPspechar_cn':function()
     {
          ...
          for(var i = 0; i < s.length; i++) 
          {
              ...
              else if(s.charAt(i).match(/[\u0391-\uFFE5]/) || s.charAt(i).match(/[\u00B7]/)
                  || s.charAt(i).match(/[\u0022]/) || s.charAt(i).match(/[\u0026]/)
                  || s.charAt(i).match(/[\u003E]/) || s.charAt(i).match(/[\u003C]/)//<>
                  || s.charAt(i).match(/[\u005C]/) || s.charAt(i).match(/[\u0027]/)
                  || s.charAt(i).match(/[\u0028]/) || s.charAt(i).match(/[\u0029]/)//()
     ...
     'domain':function()
     {
        ...
        if(!postfix.test(this) && !postfix1.test(this) && !postfix2.test(this))
            {
                return false;
            }
        return true;
     ...
     'ddnsdomain':function()
     {
         if (!cbi_validators.domain.apply(this))
             if (typeof(cbi_validators.ipv4.apply(this)) == "string")
                 return error_msg[22][1];
         return true;
     },

     'ddnscheckspecialchar':function()
     {
         var s = "" + this;
        for(var i = 0; i < s.length; i++)
        {
...
function cbi_d_update(cur_this) {
    change_hidden_input_next_span_bk(cur_this);
    init_hidden_input();
    var state = false;
    for (var i=0; i<cbi_d.length; i++) {
        var entry = cbi_d[i];
...
function cbi_validate_form(form, errmsg,errorfunction)
{
    ...
    if( form.cbi_validators )
    {
        for( var i = 0; i < form.cbi_validators.length; i++ )
        {
            ...
            var info = validator();
            if ((typeof(info)==="string") && errorfunction)
            {                        
                errorfunction(info);
                return false;
            }
...
function cbi_validate_field(cbid, optional, type, fieldtitle, error_msg)
{
