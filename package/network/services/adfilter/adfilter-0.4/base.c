...
static int base_init()
{
	...
	// watch child's crash and meminfo, if mem use too large, kill it
	if (0 != watch_dog()) goto fail;
...
static int watch_dog() {
	...
	do {
		int pid = fork();
		if (pid < 0) {
			log_error(LOG_ERR, "watch dog fork failed");
			return -1;
		}
		if (pid == 0) return 0;
        ...
        while (true) {
            ...
            if (gotPid == -1 && errno == EINTR) {
                log_error(LOG_ERR, "waitpid interrupted, retrying\n");
            } else {
                break;
            }
        }
        if (gotPid != pid) {
            log_error(LOG_ERR, "waitpid failed: wanted %d, got %d: %s\n",
		...
		nCrashCnt++;
		report_crash();
		log_error(LOG_ERR, "--- running child %d END   --- status=0x%04x, process terminated\n", pid, status);
	}
	while (nCrashCnt < 5);
	log_error(LOG_ERR, "crash too many times endit");
	return -1;
}
