...
extern relay_subsys http_filter_subsys;
...
static int redsocks_onexit(parser_section *section)
{
	...
	instance->config.bindaddr.sin_port = htons(DEFAULT_LOCAL_PORT);
	
	struct sockaddr_in* localAddr = getbrlan_addr();
	if (localAddr) {
		instance->config.bindaddr.sin_addr = localAddr->sin_addr;
	} 
	...
	if (instance->config.type) {
		...
		FOREACH(ss, relay_subsystems) {
			if (!strcmp((*ss)->name, instance->config.type)) {
				log_error(LOG_INFO, "find instance name: %s", instance->config.type);
...
static void redsocks_relay_writecb(redsocks_client *client, struct bufferevent *from, struct bufferevent *to)
{
	...
	if (EVBUFFER_LENGTH(from->input) == 0 && from_eof) {
		if (from == client->relay) {
			// client want data, but server close already, and cached data has sent to client
			redsocks_drop_client(client);
		} else {
			// server want data, but client close already, 
...
int redsocks_start_relay_ex(redsocks_client *client)
{
	int error;
	if (client->instance->relay_ss->fini)
		client->instance->relay_ss->fini(client);
	...
	client->client->readcb = redsocks_relay_clientreadcb;
	client->client->writecb = redsocks_relay_clientwritecb;
	client->client->errorcb = redsocks_event_error;
	client->relay->readcb = redsocks_relay_relayreadcb;
	client->relay->writecb = redsocks_relay_relaywritecb;
	client->relay->errorcb = redsocks_event_error;
...
static void redsocks_event_error(struct bufferevent *buffev, short what, void *_arg)
{
	...
	if (what == (EVBUFFER_READ|EVBUFFER_EOF)) {
		// read closed but  can write still
		...
		if (antiev == NULL) {
			// read eof and antiev not exist, so we release is
			redsocks_drop_client(client);
		} else if (buffev == client->relay) {
			// server says it has no more data, we do not need to write data to client
			// but the server will get more data from client for post method
			// but we guess the server will not close before post data is recived
			// we just consider that remain data has to send to client
			if (EVBUFFER_LENGTH(antiev->output) == 0) {
				redsocks_drop_client(client);
			} else {
				// wait for the writecb of the client
			}
		}
		else if (EVBUFFER_LENGTH(antiev->output) == 0) {
			// client says it has no more data, we do not need to write data to server
			// but the client will get more data from server
...
fail:
	log_error(LOG_INFO, "process client accept failed");
	if (client) {
		redsocks_drop_client(client);
	}
	if (client_fd != -1)
...
static void redsocks_debug_dump_instance(redsocks_instance *instance, time_t now)
{
	...
	list_for_each_entry(client, &instance->clients, list) {
		...
		redsocks_log_error(client, LOG_DEBUG, "client: %i (%s)%s%s, relay: %i (%s)%s%s, age: %li sec, idle: %li sec. state: %d",
			client->client ? EVENT_FD(&client->client->ev_write): 0,
			client->client? redsocks_event_str(client->client->enabled): " ",
			s_client_evshut[0] ? " " : "", s_client_evshut,

			client->relay ? EVENT_FD(&client->relay->ev_write) : 0,
			client->relay ? redsocks_event_str(client->relay->enabled): " ",
...
static int redsocks_init_instance(redsocks_instance *instance)
{
	...
	log_error(LOG_INFO, "redsocks_init_instance ok");
	
	if (enable_filter_firewall(&instance->config.bindaddr)) {
		log_error(LOG_INFO, "enable filter firewall ok");
	} else {
...
fail:
	log_error(LOG_INFO, "redsocks_init_instance failed");
...
static void redsocks_fini_instance(redsocks_instance *instance) {
	...
	log_error(LOG_INFO, "disableing filter firewall");
...
static int redsocks_init() {
	...
	log_error(LOG_INFO, "redsocks init ok");
	return 0;

fail:
	log_error(LOG_INFO, "redsocks init failed");
